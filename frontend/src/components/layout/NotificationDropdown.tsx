...
export function NotificationDropdown() {
  ...
  const alerts = logs.filter(log => log.status === 'BLOCKED' || log.status === 'REDACTED').slice(0, 5);
          ...
          >
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {alerts.length === 0 ? (
                ...
                alerts.map(alert => (
                  <div key={alert.id} className="p-4 border-b border-white/5 hover:bg-slate-800/50 transition-colors flex items-start space-x-3">
                    <div className={`p-2 rounded-lg flex-shrink-0 ${alert.status === 'BLOCKED' ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
                      {alert.status === 'BLOCKED' ? <ShieldAlert className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-200">
                        {alert.status === 'BLOCKED' ? 'Threat Blocked' : 'Data Redacted'}
                      </p>
                      <p className="text-xs text-slate-400 mt-0.5">
                        {alert.threat_type.replace('_', ' ')} detected in prompt.
                      </p>
            ...
            <div className="p-2 border-t border-white/10 bg-slate-950/50">
              <Link to="/app/logs" onClick={() => setIsOpen(false)}>
