                  ...
                  <div className="relative flex-1">
                    <div className="bg-[#0d1117] border border-white/10 rounded-md px-3 py-2 font-mono text-sm text-slate-300 flex items-center justify-between">
                      <span className="truncate mr-2">
                        {showKey[apiKey.id] || apiKey.status === 'revoked' ? apiKey.key : `sk_live_${'*'.repeat(24)}`}
                      </span>
                      <div className="flex items-center space-x-1 shrink-0">
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-400 hover:text-slate-200" onClick={() => toggleShowKey(apiKey.id)} disabled={apiKey.status === 'revoked'}>
                          {showKey[apiKey.id] ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                        </Button>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-400 hover:text-slate-200" onClick={() => handleCopy(apiKey.key, apiKey.id)} disabled={apiKey.status === 'revoked'}>
